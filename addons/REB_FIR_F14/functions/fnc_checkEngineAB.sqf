...
if !(alive _vehicle) exitWith {
	...
	if (_isNowAB) then {
		...
		if !(_unitInfo isEqualTo "") then {
			(_unitInfo displayCtrl 9908) ctrlSetText "REB_FIR_F14\ui\FA18_Toggles_clear_ca.paa";
		};
	};
};
...
if (!_isNowAB && _isThrottleAB) exitWith {
	...
	if !(_unitInfo isEqualTo "") then {
		(_unitInfo displayCtrl 9908) ctrlSetText "REB_FIR_F14\ui\FA18_Toggles_AfterBurner_ON_ca.paa";
	};
};
